...
### L1 gate results (verified 2026-07-24)

Installed (resolved) versions, confirmed against a running `npm run lint`:
...
- **Next flat exports confirmed:** `eslint-config-next/core-web-vitals` is a flat array (includes `next`, `next/typescript`, and Next's default ignores); `eslint-config-next/typescript` is a flat array (`typescript-eslint/base` + `eslint-recommended` + `recommended` + Next's TS rules). Both spread directly into the flat config — no `FlatCompat`.
- **`typescript-eslint` alongside — yes.** `eslint-config-next@16` bundles `typescript-eslint@^8.46.0` as a direct dependency, so the plugin/parser are version-aligned and the `@typescript-eslint` plugin is already registered by the `typescript` preset (the rules block references it, does **not** re-register it). `typescript-eslint` is still installed as a direct devDependency so `eslint.config.mjs` can import its `parser` reliably.
- **ESLint 9 vs 10:** latest ESLint is 10.x and both `eslint-config-next@16` (peer `eslint: >=9`) and `typescript-eslint@8` (peer `eslint: ^9 || ^10`) accept it, but this build pins **9.x** to stay faithful to this design. A bump to 10 is a low-risk future change.
- **`no-misused-promises` on JSX handlers:** the strict default flags async `onSubmit`/`onClick` handlers (a promise passed where `void` is expected). L1 resolved the two existing sites with a `void` wrapper rather than loosening the rule. If `void`-wrapping every async handler becomes friction in M2+, `checksVoidReturn: { attributes: false }` is the documented escape hatch — a conscious later decision, not a silent default.
- **Prettier scope:** `.prettierignore` narrows Prettier to code + config. Prose Markdown (`*.md`, the `docs/` tree, the throwaway HTML prototype) and vendored files (`.agents/`, regenerated by `npx convex ai-files install`) are excluded — Prettier's Markdown reflow (e.g. `*emphasis*` → `_emphasis_`) is churn that works against the design's goal of killing *code* diff noise. Widen later if prose formatting is wanted.
